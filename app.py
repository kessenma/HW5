...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/kessenHW5db"
## Provided:
...
class UpdateButtonForm(FlaskForm):
    #newRating = StringField("What is the new rating of the list?", validators=[Required()])
...
def see_all():
    ...
    # for l in lists: 
    #     lists = ToDoList.query.filter_by(id=l.list_id).first() 
    #     all_lists.append((l.title.items.name, l.priority))
    # return render_template('all_lists.html',todo_lists=lists, form=form, form_del=form_del)
...
def delete(lst):
    d = TodoList.query.filter_by(id=lst).first()
    db.session.delete(d)
    db.session.commit()
    return redirect(url_for('all_lists'))
...
def update(item):
    ...
    if form.validate_on_submit():
        newRating = form.newRating.data
        x = ToDoList.query.filter_by(title=items).first()
        x.rating = newRating
        db.session.commit()
        flash("Updated rating of " + items)
        return redirect(url_for('index'))
    return render_template('update_info', item_name = items, form = form)
